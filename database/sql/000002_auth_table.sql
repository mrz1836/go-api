...
CREATE TABLE `auths` (
    ...
    `password_digest` varchar(512) NOT NULL DEFAULT '' COMMENT 'Password digest for authentication',
    `yubikey_digest` varchar(512) NOT NULL DEFAULT '' COMMENT 'Secure Yubikey ID',
    `yubikey_backup_digest` varchar(512) NOT NULL DEFAULT '' COMMENT 'Secure Yubikey Backup ID',
    `email` varchar(100) NOT NULL COMMENT 'Unique email address for the person',
    `email_confirm_token` char(32) NOT NULL DEFAULT '' COMMENT 'Confirm email token (encrypted email address)',
    ...
    `last_ip_address` varchar(40) NOT NULL DEFAULT '' COMMENT 'Last IP address used to login',
    `last_login_at` timestamp NULL DEFAULT NULL COMMENT 'Last login time',
    `last_user_agent` varchar(255) NOT NULL DEFAULT '' COMMENT 'Last user agent from login',
    ...
    `reset_password_token` char(32) NOT NULL DEFAULT '' COMMENT 'Reset password token',
